...
begin
    ...
    if v_case.questionnaire_response_id is not null then
        select answers
        into v_answers
        from public.questionnaire_responses
        where id = v_case.questionnaire_response_id;
    else
        v_answers := v_case.patient_facts;
    end if;
    ...
    if not found then 
        raise exception 'Investigations not found for this clinical case';
    end if;
    ...
    v_vitamin_d_level := v_investigation.vitamin_d_level;
    v_total_calcium := v_investigation.total_calcium;
    v_ionised_calcium := v_investigation.ionised_calcium;
    v_body_weight_kg := v_investigation.body_weight_kg;
